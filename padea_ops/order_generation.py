...
def _session_date(s):
    # NOTE This normalization step might be redundant, I'll verify later
    raw = s.get("session_date") or s.get("date")
...
def get_active_students_for_session(client: Client, session: dict[str, Any]) -> list[dict[str, Any]]:
    """Resolve active attendees through student_sessions.

    If the junction table is unavailable or empty for a session, fall back to
    students.school_id when present. In the current MVP, student_sessions should
    ...
    try:
        student_sessions = fetch_all(client, "student_sessions", filters={"session_id": session_id})
        active_ids = {
            row.get("student_id")
            for row in student_sessions
            if row.get("student_id") and boolish(row.get("active", True))
        }

        if active_ids:
            return [
                student for student in students
                if _student_id(student) in active_ids
                and boolish(student.get("active", True))
            ]
    except Exception:
        pass
    ...
    return [
        student for student in students
        if boolish(student.get("active", True))
        and student.get("school_id") == school_id
    ]
...
def generate_for_session(client: Client, session: dict[str, Any]) -> GeneratedOrder:
    ...
    if full_exclusion:
        order = {"order_id": order_id, "session_id": session_id, "caterer_id": caterer_id,
...
def replace_generated_outputs(client: Client, orders: list[GeneratedOrder]) -> None:
    for g in orders:
        ...
        for table in ["order_lines", "orders"]:
